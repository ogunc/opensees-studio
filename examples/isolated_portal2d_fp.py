"""Base-isolated portal frame on two single friction pendulum bearings.

The ISO-1 example (``isolated_portal2d.py``: the Example 1b elastic portal
frame in inch, kip units on two isolators, base-slab mass and equalDOF,
BM68elc scaled with the project unit system after a gravity preload) with
``singleFPBearing`` isolators instead of the elastomeric ones, same record
and same scale, so the two responses can be compared side by side.

Isolator design (per bearing): the frame weight is about 2000 kip per
column line. A Coulomb friction coefficient of 0.05 gives the same
characteristic strength as the ISO-1 Qd of 100 kip, and the effective
radius Reff = 61 in gives the pendulum period 2 pi sqrt(Reff / g) of about
2.5 s, the post-yield period of the ISO-1 design. The stick stiffness
Kinit = 450 kip/in (as ISO-1) puts the slip displacement mu W / Kinit at
about 0.22 in.

Run from the repository root:

    python examples/isolated_portal2d_fp.py

Produces ``examples/isolated_portal2d_fp.osmodel``.
"""
...
# Sliding bearing, per isolator
K_INIT = 450.0  # kip/in (stick stiffness before sliding)
MU = 0.05  # Coulomb friction coefficient
R_EFF = 61.0  # in, effective radius: 2 pi sqrt(Reff / g) is about 2.5 s
FRICTION_MODEL_ID = 1
...
def _bearing(element_id: int, ground: int, base: int, name: str) -> SingleFPBearingElement:
...
def build_isolated_portal2d_fp() -> Project:
    ...
    return Project(
        meta=ProjectMeta(
            name="Base-isolated portal frame (Ex 1b on single FP bearings)",
            author="OpenSees Studio examples",
            description=(
                "Example 1b elastic portal frame on two singleFPBearing isolators with "
                "Coulomb friction: gravity preload then the BM68elc record as uniform "
                "excitation. Companion of isolated_portal2d (elastomeric bearings)."
            ),
        ...
        friction_models=[CoulombFriction(id=FRICTION_MODEL_ID, name="PTFE", mu=MU)],
        sections=[
            ElasticSection(
...
def main() -> None:
    project = build_isolated_portal2d_fp()
    project.validate_references()
    bearing = project.element(BEARING_L)
    weight = -GRAVITY_W * L_BEAM / 2.0  # tributary weight per bearing (kip)
    print(f"Built '{project.meta.name}'")
    print(f"  Isolators: Kinit {K_INIT} kip/in, mu {MU}, Reff {R_EFF} in")
    print(
        f"  Slip displacement {bearing.yield_displacement(MU, weight):.4f} in, "
        f"friction force {MU * weight:.2f} kip, restoring stiffness "
        f"{bearing.restoring_stiffness(weight):.3f} kip/in, isolated period "
        f"{bearing.isolated_period(project.meta.units):.3f} s"
    )
